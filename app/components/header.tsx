...
export default function Header() {
  ...
  const sidebarVisible = useSelector((state: RootState) => state.sidebar.isOpen);//Pegando o true ou false do isopen do sidebar
      ...
      <div className="flex items-center space-x-4">
        ...
        >
          ...
          <input
            type="text"
            placeholder="Buscar..."
            className={`absolute right-0 top-1/2 -translate-y-1/2 bg-[var(--color-bg-alt)] border border-[var(--color-border)] text-[var(--color-text)] placeholder-[var(--color-muted)] rounded-md px-3 py-1 transition-all duration-300 ease-in-out ${
              showSearchMobile ? "w-66 opacity-100" : "w-0 opacity-0"
            }`}
            style={{ zIndex: 5 }}
          />
        </div>
